...
It queries *witnesses* through `mosaika` by *entry address*.
Humans should run `sirno witness ENTRY_ADDRESS --full` to inspect evidence.
...
A *repository witness* block opens with `sirno:witness:<entry-address>:begin`
and closes with `sirno:witness:<entry-address>:end`.
The opening and closing *entry addresses* must match.
...
The convention is the *entry address* plus the *repository witness* block.
...
The *entry address* ties them together.
...
Reusing a near-enough *entry address* makes review less precise.
