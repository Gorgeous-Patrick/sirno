...
The reservoir is the required canonical working form.
It stores flat Markdown *entries* at `.sirno/lake`
and lake-owned *entry artifacts* under `.sirno/lake/.artifacts`.
Configured repo members are optional and enable *witness* lookup.
Sirno control files live under `.sirno/` next to `Sirno.toml`.
Git stores history for the reservoir and control files.

The reservoir and mist design separates canonical storage from visible projection.
