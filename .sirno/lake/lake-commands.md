---
desc: The command surface for initializing and moving a Sirno Lake.
name: Lake Commands
category:
  - concept
  - implemented
...
They keep reservoir setup and path movement on the lake boundary.
Entry-level mutations stay under entry commands.
Projection work stays under mist commands.
Read-only lake health belongs to project status.
...
`project-status-commands` owns lake health reporting.
