...
Sirno uses the *entry address* as the *witness* query key,
...
A *witness* block opens with `sirno:witness:<entry-address>:begin`
and closes with `sirno:witness:<entry-address>:end`.
Both sentinels name the same *entry address*.
...
The shared key is the *entry address*.
